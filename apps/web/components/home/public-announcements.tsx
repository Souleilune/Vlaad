    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-5">
      <Card className="overflow-hidden p-0">
        <div className="grid gap-0 lg:grid-cols-[0.95fr_1.05fr]">
          <div className="bg-[linear-gradient(180deg,rgba(255,245,218,0.8),rgba(255,255,255,0.9))] p-6 sm:p-8">
            <Badge className="bg-retroYellow/50 text-slate-800">Patch Notes</Badge>
            <h2 className="mt-3 text-3xl font-semibold text-slate-900">Recent platform-wide updates.</h2>
          ...
          <div className="grid gap-4 p-6 sm:p-8">
            {announcements.slice(0, 4).map((item) => (
              <article key={item.id} className="rounded-[24px] border border-slate-200 bg-white/85 p-5 shadow-[0_10px_30px_rgba(15,23,42,0.06)]">
                <div className="flex items-center justify-between gap-3">
                  <Badge className="bg-softCoral/12 text-slate-800">{item.label}</Badge>
                  <p className="text-xs uppercase tracking-[0.18em] text-slate-400">{formatAnnouncementDate(item.createdAt)}</p>
                </div>
